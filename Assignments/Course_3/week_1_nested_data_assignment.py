# David Campos
# October, 2020
# Python3, Course 3 Specialization
...
# or what they were asking for:
output = nested[1][2] # but what if you could not count the positions?
...
test1 = "hola" in L # it is within another list but not in the list "L"

# Test if [5, 8, 7] is in the list L. Save to variable name test2
num_list = [5, 8, 7]
test2 = all(item in num_list for item in L[0])
...
# 5.The variable nested_d contains a nested dictionary with the gold medal
...
nested_d = {
    'Beijing':{
        'China':51,
               'USA':36, 'Russia':22, 'Great Britain':19
    },
    'London':{
...
# Empty List to Append any Values (other than USA)
usa_medals = []
for olympics in nested_d:
    for country in nested_d[olympics]:
        if country == "USA":
            usa_medals.append(nested_d[olympics][country])
...
third = []
for items in l_of_l:
    third.append(items[2])

# or even better:
third = [i[2] for i in l_of_l]


# 9. Given below is a list of lists of athletes. Create a list, t,
